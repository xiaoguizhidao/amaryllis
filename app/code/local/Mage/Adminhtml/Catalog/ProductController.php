...
class AS_Banner_Catalog_ProductController extends Mage_Adminhtml_Catalog_ProductController
{
    ...
    public function editAction()
    {
        ...
        $paramcategory = explode("-",$product->getName()); 
        $_product = Mage::getModel('catalog/product')->load($productId);
        $ids = $_product->getCategoryIds();
        //$category1 = Mage::getModel("catalog/category")->load($ids[0]);
        //print_r($ids);
        foreach($ids as $_cat)
        {
            $category = Mage::getModel("catalog/category")->load($_cat);
            $catname = $category->getName();
            //echo $catname. "==". $paramcategory[0]; 
            if($catname == $paramcategory[0])
            {
                $productname = $paramcategory[1];
                $product->setName($productname);
                
            }
            else if($paramcategory[0] == "")
            {
                $productname = $paramcategory[1];
                $product->setName($productname);
            }    
...
//            }
    ...
    protected function _initProductSave()
    {
        ...
        $trim = ltrim($categoryIds,",");
        
        //echo count(explode(",",$categoryIds)); die();
        $catname = "";
        $categories = explode(",",$trim);
        ...
        if($trim!="")
        {    
        //echo $catname."-".$product->getName();die();
        $product->setName($catname."-".$product->getName());
        }
        else
        {
