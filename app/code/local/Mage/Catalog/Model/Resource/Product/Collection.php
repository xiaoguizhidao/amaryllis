...
class Mage_Catalog_Model_Resource_Product_Collection extends Mage_Catalog_Model_Resource_Collection_Abstract
{
    ...
    protected function _afterLoad()
    {
        ...
        foreach ($this as $product) 
        {
            ...
            //code for product rename start
            
            $productmodel = Mage::getModel("catalog/product")->load($product["entity_id"]);
            $catids = $productmodel->getCategoryIds();
            $categoryName = null;
            foreach($catids as $_catid)
            {
                $categorymodel = Mage::getModel("catalog/category")->load($_catid);
            ...
            {
                $proName = $productmodel->getName();
                
                if(strstr($proName, $categoryName." - "))
                {
                    if($area != "frontend")
                    {
                        $proName = str_replace($categoryName." - ","", $proName);
                    }
                    else
                    {
                        // for front
                        if(($modulename == "catalog" )&& ($controllername == "product" || $controllername == "category"))
                        {
                            $proName = str_replace($categoryName." - ","", $proName);
                        }
                    }
