...
abstract class Fooman_PdfCustomiser_Helper_Pdf extends Mage_Core_Helper_Abstract
{
    ...
    public function getDeliveryDate(){
        ...
        $table = $resource->getTableName('deliverydate/delivery');

        //$orderobject = Mage::getModel('sales/order')->load($this->getSalesObject()->getIncrementId(), 'increment_id');
        //$theorderid = $this->getSalesObject()->getId();
        //echo $this->getSalesObject()->getOrderId() ;
        $theorderid = $this->getOrder()->getId();
        $query = 'SELECT delivery_date FROM ' . $table . ' WHERE order_id = '. (int)$theorderid . ' LIMIT 1';
        //echo $this->getOrder()->getId();
        $deliverydate = $readConnection->fetchOne($query);

        echo $this->getOrder()->getAwDeliverydateDate();

        if($deliverydate !=''){
            $date   = new Datetime($deliverydate);
            //return $date->format('M d, Y');
            //return $deliverydate;
            $pieces = explode('-', $deliverydate);
            $year = $pieces[0];
            $month = $pieces[1];
            $day = $pieces[2];
        ...
        */
        //echo $this->getOrder()->getAwDeliverydateDate();
        $deliverydate = $this->getOrder()->getAwDeliverydateDate();
        if($deliverydate !=''){
            //$date = Mage::helper('core')->formatDate($this->getOrder()->getAwDeliverydateDate(), 'medium', false);
            $description = $this->getOrder()->getAwDeliverydateNotice();
            $date   = new Datetime($deliverydate);
            //return $date->format('M d, Y');
            //return $deliverydate;
            $pieces = explode('-', $deliverydate);
            ...
            $dateletter = $monthName.' '.$day.', '.$year;
            return $dateletter .'<br/>'.$description;
        }else{
            return '---';
        }
